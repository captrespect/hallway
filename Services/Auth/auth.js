...
myOAP.on('enforce_login', function(req, res, authorize_url, next) {
  ...
  // , function(err, auth, compReq, compRes) {
  //     if(err) return compRes.send(err, 500);
  //     compReq.session.user = {};
  //     compReq.session.user[provider].auth = auth;
  //     next(compReq, compRes, compReq.session.user);
  //   });
