...
var myOAP = new OAuth2Provider(lconfig.authSecret.crypt, lconfig.authSecret.sign);
...
//  res.end('<html>this app wants to access your account... <form method="post" action="' + authorize_url + '"><button name="allow">Allow</button><button name="deny">Deny</button></form><script>document.forms[0]["allow"].click()</script>');
  res.end('<form method="post" action="' + authorize_url + '"><input name="allow" value="true"></form><script>document.forms[0].submit()</script>');
});
...
myOAP.on('lookup_grant', function(client_id, client_secret, code, callback) {
