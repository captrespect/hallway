...
function startServiceAuth(service, appID, req, res) {
  ...
  if (authModule.authUrl) return apiKeys(service, appID, function(keys) {
    ...
    var optApp = (appID) ? "?app="+appID : ""; // optionally pass app back along
    var url = authModule.authUrl + '&client_id=' + keys.appKey + '&redirect_uri=' + lconfig.externalBase + '/auth/' + service + '/auth'+optApp;
...
exports.authIsAuth = function(service, appID, req, res) {
  ...
  apiKeys(service, appID, function(keys){
    ...
    if (typeof authModule.handler === 'function') {
      try {
        var optApp = (appID) ? "?app="+appID : "";
        return authModule.handler(host + '/auth' + service + '/auth'+optApp, keys, function (err, auth) {
    ...
    finishOAuth2(code, service, authModule, keys, function (err, auth) {
      if (err) return res.send(err, 500);
      finishAuth(req, res, service, auth);
    });
...
function finishOAuth2(code, service, authModule, theseKeys, callback) {
