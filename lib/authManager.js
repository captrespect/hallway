...
function startServiceAuth(service, appID, req, res) {
  console.error('starting service auth for '+service,appID);
  var authModule;
  try {
  ...
  if (authModule.authUrl) return apiKeys(service, appID, function(keys) {
    ...
    var url = authModule.authUrl + '&client_id=' + keys.appKey + '&redirect_uri=' + lconfig.externalBase + '/auth/' + service + '/auth/'+appID;
...
exports.authIsAuth = function(service, appID, req, res) {
  ...
  var redirectURI = lconfig.externalBase + '/auth/' + service + '/auth/'+appID;
    ...
    acl.getOrAdd(acookie.id, req.session.app, auth.pid, function(err, account) {
      ...
      async.series([
        ...
        function() {
          ...
          if(!req.session.loggedInUrl) return res.send('session failure, mea culpa', 500);
