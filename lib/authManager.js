...
function authfail(req, service, res, E, app)
{
  ...
  if(!req.cookies || !req.cookies.callback) res.send(E, 500);
  var url = urllib.parse(req.cookies.callback,true);
  if(!url || !url.query || !url.query.redirect_uri) res.send(E, 500);
