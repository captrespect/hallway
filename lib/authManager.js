...
function finishAuth(req, res, service, auth) {
  ...
  self.sync({auth:auth}, function(err, data){
    ...
    var acookie = getAccountCookie(req, req.session.app);
    accountsManager.getOrAdd(acookie.id, req.session.app, auth.pid, function(err, account) {
...
function getAccountCookie(req, app)
{
...
//console.error("ENFORCE "+req.url+" "+JSON.stringify(req.session)+" "+JSON.stringify(req.query));
  ...
  accountsManager.getApp(req.query.client_id, function(err, app){
    ...
    // if there's a sticky account cookie, see if we can skip the auth stuff
    if(req.cookies && req.cookies["account-"+app.app])
    {
      var account;
      try {
        account = myOAP.serializer.parse(req.cookies["account-"+app.app]);
      }catch(E){}
      // TODO, someday there will need to be an override to request connecting ANOTHER profile for this service, two twitter accounts, etc
      if(account && account.services && account.services[req.query.service]) return cbForm(account.id);
    }
...
//  console.error("SAVEGRANT "+JSON.stringify(req.session));
  var account = getAccountCookie(req, client_id);
...
myOAP.on('access_token', function(req, token, next) {
  ...
  req._singly_auth = {account:token.user_id, app:token.client_id};

  accountsManager.getProfiles(req._singly_auth.account, function(err, profiles) {
    if(err) return next('complain loudly! '+err);
    req._singly_auth.profiles = profiles;
    next();
  });
});
