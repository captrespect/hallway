...
function authfail(service, res, E)
{
...
function startServiceAuth(service, appID, req, res) {
  ...
  if (authModule.authUrl) {
    return apiKeys(service, appID, function(keys) {
      if (!keys) return authfail(service, res, 'missing required oauth2 api keys', 500);
...
exports.authIsAuth = function(service, appID, req, res) {
  ...
  try {
    ...
    return authfail(service, res, E, 500);
  }
  ...
  if (typeof authModule.direct === 'function') {
    try {
      ...
      return authfail(service, res, err, 500);
    }
  }
  ...
  apiKeys(service, appID, function(keys){
    if (!keys) return authfail(service, res, 'missing required api keys', 500);

    if (typeof authModule.handler === 'function') {
      try {
        return authModule.handler(redirectURI, keys, function (err, auth) {
          if (err) return authfail(service, res, err, 500);
    ...
    if (!code || !authModule.handler.oauth2) return authfail(service, res, 'missing auth code, doth deny us? :(', 500);
...
function finishAuth(req, res, service, auth, appID) {
  ...
  self.sync({auth:auth, config:{}}, function(err, data){
    ...
    acl.getOrAdd(acookie.id, appID, auth.pid, function(err, account, count) {
      ...
      if (err) return authfail(service, res, err, 500);
      if (!account) return authfail(service, res, 'could not create a user', 500);
      ...
      async.series([
        function(cb) {
          profileManager.authSet(auth.pid, auth, cb);
        },
        function() {
          ...
          if(!redir) return authfail(service, res, 'cookies disabled? missing redirect :(', 500);
...
/*      req.query.redirect_uri = req.query.redirect_uri.replace(/</g, '&lt;');
      return res.send(
        "Unacceptable redirect_uri. If you are the developer, please check " +
        "<pre>" + req.query.redirect_uri + "</pre>" +
        " against your " +
    ...
    acl.getProfiles(account.id, function(err, profiles) {
      ...
      return found ? cbForm(account.id) : auther();
    });
  });
});
