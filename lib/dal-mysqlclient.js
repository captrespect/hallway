...
Db.prototype.query = function(sql, binds, cbDone) {
  ...
  var client = (sql.toLowerCase().indexOf("select") == 0 && this.slave.connectedSync()) ? this.slave : this.client;
...
Db.prototype.connect = function(options, cbDone) {
  ...
  this.client.connect(options.host, options.user, options.password, options.database, function(error) {
    if(error || !options.slavehostname) return cbDone(error, self);
    this.slave.connect(options.slavehost, options.user, options.password, options.database, function(error) {
      cbDone(error, self);
    });
  });
};
