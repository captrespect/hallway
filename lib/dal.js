...
pool.query = function(sql, binds, cbDone) {
  ...
  this.acquire(function(error, db) {
    ...
    if (binds) {
      logger.debug("Matching binds");
      // XXX: Hacks specific to mysqlclient!
      sql = sql.replace(/\?/g, function() { var arg = binds.shift(); return "'" + db.client.escapeSync(arg.toString()) + "'"; });
    }
    ...
    db.query(sql, function(error, rows) {
      self.release(db);
      if (error) {
        return cbDone(new Error(error));
      }

      return cbDone(error, rows);
    });
    return {sql:sql};
  });
}
