...
{
  var self = this;
  if((req && typeof req != 'object') || (js && typeof js != 'object')) return self.warn("anubis called w/ invalid args");
  if(!req) req = {}; // if null
  if(!js) js = {};
  if(!(js.app||js.act) && !req._authsome) return self.warn("anubis isn't authsome");
  // fill in log entry
  ...
  if(req.method && req.method.toLowerCase() != 'get') js.method = req.method.toLowerCase();
  if(js.path && js.path.indexOf('?') != -1) js.path = js.path.substr(0, js.path.indexOf('?'));
