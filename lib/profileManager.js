...
function genGet(fields, id, callback) {
  dal.acquire(function(err, db) {
    if(err) return callback(err);
    var q = db.query("SELECT "+fields.join(",")+" FROM Profiles WHERE id = ? LIMIT 1", [id], function(err, rows) {
      if(err || !rows) logger.error("now rows or error ",err,q.sql);
      rows = rows || [{}];
      dal.release(db);
      var ret = {};
      // parse each field returned into an object
      for(var i = 0; i < fields.length; i++) try {
      ...
      var q = db.query("INSERT INTO Profiles (id, service, worker) VALUES (?, ?, ?)", [id, parts[1], lconfig.workerName], function(err2) {
        if(err2) logger.error("query failed: "+q.sql+" ", err2);
  ...
  genGet([field], id, function(err, old) {
    ...
    dal.acquire(function(err, db) {
      ...
      var q = db.query("UPDATE Profiles SET `"+field+"` = ? WHERE id = ?", [val, id], function(err){
        dal.release(db);
        if(err) logger.error("query failed: "+q.sql+" ", err);
        callback(err);
      });
    });
  });
