...
function genGet(field, id, callback) {
  dal.acquire(function(err, db) {
    if(err) return callback(err);
    db.query("SELECT ? FROM Profiles WHERE id = ? LIMIT 1", [field, id], function(err, rows) {
      rows = rows || [];
      dal.release(db);
      var js = {};
      try {
        js = JSON.parse(rows[0]);
      } catch(E) {}; // rows is often empty, so always return a valid container object
      // catch if there's no entry yet and make sure there is one so that UPDATE syntax works!
      if(rows.length == 1) return callback(err, js);
      var parts = id.split('@');
      var q = db.query("INSERT INTO Profiles (id, service, worker) VALUES (?, ?, ?)", [id, parts[1], lconfig.workerName], function(err2) {
        ...
        callback(err, js); // return original now
      })
    });
  });
}
...
function genSet(field, id, val, callback) {
  genGet(field, id, function(err, old) {
    ...
    if(typeof val == 'object')
    {
      ...
      val = JSON.stringify(lutil.extend(true, old, val));
    }
    dal.acquire(function(err, db) {
      if(err) return callback(err);
...
exports.authGet = function(id, callback) { genGet('auth', id, callback) }
exports.authSet = function(id, js, callback) { genSet('auth', id, js, callback) }
exports.configGet = function(id, callback) { genGet('config', id, callback) }
