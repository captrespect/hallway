...
exports.init = function(callback) {
  ...
  dal.acquire(function(err, db) {
    ...
    var creates = [
      "CREATE TABLE IF NOT EXISTS Accounts (id INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY, account VARCHAR(255), app VARCHAR(255), profile VARCHAR(255), token VARCHAR(255))",
      "CREATE TABLE IF NOT EXISTS Apps (`app` VARCHAR(255) PRIMARY KEY, secret VARCHAR(255), notes TEXT)"
    ];
...
exports.getAppProfile = function(app, profile, callback) {
  ...
  dal.acquire(function(err, db) {
    if(err) return callback(err);
    db.query("SELECT account,token FROM Accounts WHERE app = ? AND profile = ? LIMIT 1", [app, profile], function(err, rows) {
      rows = rows || [];
      dal.release(db);
      callback(err, rows[0]);
    });
...
// save the new token for anything on a given account
exports.setToken = function(account, token, callback) {
  if(!account || !token) return callback(new Error("bad args"));
  dal.acquire(function(err, db) {
    if(err) return callback(err);
    db.query("UPDATE Accounts SET token = ? WHERE account = ?", [token, account], function(err) {
      dal.release(db);
      callback(err);
    });
  });
...
exports.getApp = function(app, callback) {
  ...
  dal.acquire(function(err, db) {
    if(err) return callback(err);
    db.query("SELECT secret FROM Apps WHERE app = ? LIMIT 1", [app], function(err, rows) {
      rows = rows || [];
      dal.release(db);
      callback(err, rows[0]);
    });
