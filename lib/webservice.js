...
locker.get('/auth/:id/auth', function(req, res) {
  authManager.authIsAuth(req.params.id, req.query.app, req, res);
});
...
locker.get('/profiles', function(req, res) {
  ...
  if(!profiles) return res.json('no profiles found', 500);
  var ret = {all:[]};
...
locker.post('/profiles/delete', function(req, res) {
  ...
  if(!account) return res.json('no account', 500);
...
locker.get('/types/:type', function(req, res) {
  ...
  if(!req._authsome.profiles) return res.json('no profiles found', 500);
  var profiles = [];
...
locker.get('/services/:serviceName/:serviceEndpoint', function(req, res) {
  ...
  if(!pid) return res.json('missing profile for '+service, 500);
