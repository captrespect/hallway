  ...
  dal.acquire(function(error, db) {
    ...
    db.query(sql, binds, function(error, rows) {
      ...
      function processPath(cbPath) {
        ...
        appendRange(start, end);
        console.log("Getting range: %s", rangeStr);
        var req = s3client.get(curPath, {
          ...
          function nextRange() {
            if (curBuf) {
              console.log("Processing buf with length %d", curRange.len);
          ...
          nextRange();
          res.on("data", function(chunk) {
            fs.writeFileSync("chunk.gz", chunk);
            console.log("Chunk length(%d) curRange len(%d) writePos(%d)", chunk.length, curRange.len, curRange.writePos);
            if (chunk.length <= curRange.len - curRange.writePos) {
              chunk.copy(curBuf, curRange.writePos);
              curRange.writePos += chunk.length;
              ...
              while (chunkLeft > 0){
                ...
                if (chunkLeft > writeLen) {
                  console.log("Copying %d from %d to %d", writeLen, chunk.length - chunkLeft, curRange.writePos);
                  chunk.copy(curBuf, curRange.writePos, chunk.length - chunkLeft, writeLen);
                  chunkLeft -= writeLen;
                  nextRange();
                } else {
                  chunk.copy(curBuf, curRange.writePos, chunk.length - chunkLeft, chunkLeft);
