...
IJOD.prototype.addData = function(arg, callback) {
  ...
  this.startAddTransaction(function() {
    ...
    dal.query("INSERT INTO ijod (idr, base, path, hash, at, offset, len) VALUES (?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE path=VALUES(path), hash=VALUES(hash), at=VALUES(at), offset=VALUES(offset), len=VALUES(len)", [idr.hash(arg.idr), idr.baseHash(arg.idr), self.path, hash, arg.at, offset, (self.len - offset)], function(err){
      ...
      async.forEachSeries(Object.keys(arg.types), function(type, cb){
        ...
        dal.query("INSERT INTO ijod (idr, base, path, hash, at, offset, len) VALUES (?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE path=VALUES(path), hash=VALUES(hash), at=VALUES(at), offset=VALUES(offset), len=VALUES(len)", [idr.hash(i2), idr.baseHash(i2), self.path, hash, arg.at, offset, (self.len - offset)], cb);
      }, callback);
    });
  });
}
