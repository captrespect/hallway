...
IJOD.prototype.addData = function(arg, callback) {
  ...
  var qx = ['00','00','00','00'];
  var buf = qix.buf(q);
  logger.warn("Q",arg.idr,q,buf&&buf.toString('hex'));
  if(buf)
  {
  ...
  this.startAddTransaction(function() {
    ...
    async.forEachSeries(Object.keys(arg.types), function(type, cb){
      ...
      self.transactionQueries.push({sql:"INSERT INTO ijod (idr, base, path, hash, at, offset, len, lat, lng, q0, q1, q2, q3) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, x?, x?, x?, x?) ON DUPLICATE KEY UPDATE path=VALUES(path), hash=VALUES(hash), at=VALUES(at), offset=VALUES(offset), len=VALUES(len)", binds:[idr.hash(i2), idr.baseHash(i2), self.path, hash, arg.at, offset, (self.len - offset), ll[0], ll[1], qx[0], qx[1], qx[2], qx[3]]});
      cb();
    }, callback);
  });
}
      ...
      async.map(paths, processPath, function(error, results) {
        for (var i = 0; i < results.length; ++i) {
          ...
          for (var j = 0; j < results[i].length; ++j) {
            ...
            if(range.q)
            {
              var q = qget(results[i][j]).toLowerCase();
              var parts = range.q.toLowerCase().split(/\s+/);
              var matches = 0;
              parts.forEach(function(part){if(q.indexOf(part) >= 0) matches++ });
              if(matches != parts.length)
              {
                logger.warn("couldn't find ",parts,"in",q);
                continue;
              }
            }
            cbEach(results[i][j]);
...
function qget(entry)
{
  ...
  var q = [entry.q, oe.url, oe.title, oe.author_name, dMap.get('text', entry.data, entry.idr)].join(" "); // get all queryable strings
}
