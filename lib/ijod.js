        ...
        var req = s3client.get(curPath, {
          ...
          res.on("data", function(chunk) {
            ...
            while (curRange && chunkLeft > 0){
              ...
              if (exports.debug) logger.debug("Writing curBuf writelen(" + writeLen + ") writePos(" + curRange.writePos + ") chunk startPos(" + startPos + ") chunkLeft(" + chunkLeft + ")");
              if (chunkLeft > writeLen) {
                chunk.copy(curBuf, curRange.writePos, startPos, startPos + writeLen);
                chunkLeft -= writeLen;
                nextRange();
              } else {
                logger.debug("Only " + chunkLeft + " starting from " + startPos);
                chunk.copy(curBuf, curRange.writePos, startPos);
