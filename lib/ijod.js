...
exports.initDB = function(db, cbDone) {
  db.query("CREATE TABLE IF NOT EXISTS ijod (idr VARCHAR(32) NOT NULL, base VARCHAR(32), path VARCHAR(255), hash VARCHAR(32), at INT, offset INT, len INT, PRIMARY KEY(idr))", cbDone);
}
...
exports.getOne = function(targetIdr, callback) {
  ...
  dal.acquire(function(error, db) {
    ...
    db.query("SELECT storageObject, offset, len FROM ijod WHERE idr = ? LIMIT 1", [idr.hash(targetIdr)], function(error, rows) {
      if (error) return cbError(error);
      if (rows.length != 1) return cbError(new Error("Bad query for getOne"));

      var buf = new Buffer(row.len);
      var appendPos = 0;
      s3client.get(rows[0].storageObject, {
        "Range":"bytes=" + rows[0].offset + "-" + (rows[0].offset + rows[0].len - 1)
      }).on("response", function(res) {
        if (res != 200) return cbError(new Error("Bad result from storage"));
