...
IJOD.prototype.addData = function(arg, callback) {
  ...
  this.startAddTransaction(function() {
    ...
    var sql = "INSERT INTO Entries (base, idr, path, hash, offset, len, lat, lng, q0, q1, q2, q3) VALUES (unhex(concat(rpad(?,32,'0'), lpad(hex(floor(?/1000)),8,'0'), lpad(hex(conv(substr(?,1,8),16,10)+(? - (floor(at/1000)*1000))),8,'0'))), unhex(?), ?, ?, ?, ?, ?, ?, ?, "+qsql+") ON DUPLICATE KEY UPDATE path=VALUES(path), hash=VALUES(hash), offset=VALUES(offset), len=VALUES(len), lat=VALUES(lat), lng=VALUES(lng), q0=VALUES(q0), q1=VALUES(q1), q2=VALUES(q2), q3=VALUES(q3)";
    self.transactionQueries.push({sql:sql, binds:[idr.baseHash(arg.idr), arg.at, idr.hash(arg.idr), arg.at, idr.hash(arg.idr), self.path, hash, offset, (self.len - offset), ll[0], ll[1], qx[0], qx[1], qx[2], qx[3], par]});
