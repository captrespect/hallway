...
IJOD.prototype.addData = function(arg, callback) {
  ...
  this.startAddTransaction(function() {
    ...
    dal.query("INSERT INTO ijod VALUES (?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE path=VALUES(path), hash=VALUES(hash), at=VALUES(at), offset=VALUES(offset), len=VALUES(len)", [idr.hash(arg.idr), idr.baseHash(arg.idr), self.path, hash, arg.at, offset, (self.len - offset)], function(err){
      ...
      async.forEachSeries(Object.keys(arg.types), function(type, cb){
        ...
        i2.protocol = type;
        dal.query("INSERT INTO ijod VALUES (?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE path=VALUES(path), hash=VALUES(hash), at=VALUES(at), offset=VALUES(offset), len=VALUES(len)", [idr.hash(i2), idr.baseHash(i2), self.path, hash, arg.at, offset, (self.len - offset)], cb);
      }, callback);
    });
  });
}
...
exports.getOne = function(targetIdr, callback) {
  ...
  var hash = (typeof targetIdr == 'string' && targetIdr.length == 32 && targetIdr.indexOf(':') == -1) ? targetIdr : idr.hash(targetIdr);
...
exports.getRange = function(basePath, range, cbEach, cbDone) {
  ...
  dal.query(sql, binds, function(error, rows) {
    ...
    function processPath(cbPath) {
      ...
      if (exports.debug) logger.debug("Ranges: " + rangeStr,curPath);
