...
IJOD.prototype.addData = function(arg, callback) {
  ...
  if(participants.length > 0)
  {
    ...
    participants.forEach(function(part){
      ...
      par += (part == owner) ? 'ff' : ((parseInt(mmh3.murmur32HexSync(part),16) % 254)+1).toString(16);
    });
  }else{
    qsql += ' ?';
  }
  ...
  this.startAddTransaction(function() {
    ...
    async.forEachSeries(Object.keys(arg.types), function(type, cb){
      ...
      self.transactionQueries.push({sql:sql, binds:[idr.baseHash(i2), arg.at, idr.hash(i2), idr.hash(i2), self.path, hash, arg.at, offset, (self.len - offset), ll[0], ll[1], qx[0], qx[1], qx[2], qx[3], par]});
      cb();
    }, callback);
  });
}
