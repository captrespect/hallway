...
IJOD.prototype.addData = function(arg, callback) {
  ...
  this.startAddTransaction(function() {
    ...
    dal.query("INSERT INTO ijod (idr, base, path, hash, at, offset, len, lat, lng) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE path=VALUES(path), hash=VALUES(hash), at=VALUES(at), offset=VALUES(offset), len=VALUES(len)", [idr.hash(arg.idr), idr.baseHash(arg.idr), self.path, hash, arg.at, offset, (self.len - offset), ll[0], ll[1]], function(err){
      ...
      async.forEachSeries(Object.keys(arg.types), function(type, cb){
        ...
        dal.query("INSERT INTO ijod (idr, base, path, hash, at, offset, len, lat, lng) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE path=VALUES(path), hash=VALUES(hash), at=VALUES(at), offset=VALUES(offset), len=VALUES(len)", [idr.hash(i2), idr.baseHash(i2), self.path, hash, arg.at, offset, (self.len - offset), ll[0], ll[1]], cb);
      }, callback);
    });
  });
}
      ...
      dal.query("SELECT path, offset, len FROM ijod WHERE idr = ? LIMIT 1", [hash], function(error, rows) {
        ...
        var req = s3client.get(rows[0].path, {
          ...
          res.on("end", function() {
            ...
            memcache.set(hash, data, function(error, result) {
