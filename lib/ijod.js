...
insert ignore into Entries (base, idr, path, hash, offset, len, lat, lng, q0, q1, q2, q3) select unhex(concat(rpad(binary base,32,'0'), lpad(hex(floor(at/1000)),8,'0'), lpad(hex(crc32(idr)),8,'0'))), unhex(idr), path, hash, offset, len, lat, lng, q0, q1, q2, q3 from ijod
...
IJOD.prototype.addData = function(arg, callback) {
  ...
  // build our participant matching
  var par = null;
  ...
  this.startAddTransaction(function() {
    ...
    var sql = "INSERT INTO Entries (base, idr, path, hash, offset, len, lat, lng, q0, q1, q2, q3) VALUES (unhex(concat(rpad(?,32,'0'), lpad(hex(floor(?/1000)),8,'0'), lpad(hex(crc32(?)),8,'0'))), unhex(?), ?, ?, ?, ?, ?, ?, "+qsql+") ON DUPLICATE KEY UPDATE path=VALUES(path), hash=VALUES(hash), offset=VALUES(offset), len=VALUES(len), lat=VALUES(lat), lng=VALUES(lng), q0=VALUES(q0), q1=VALUES(q1), q2=VALUES(q2), q3=VALUES(q3)";
    self.transactionQueries.push({sql:sql, binds:[idr.baseHash(arg.idr), arg.at, idr.hash(arg.idr), idr.hash(arg.idr), self.path, hash, offset, (self.len - offset), ll[0], ll[1], qx[0], qx[1], qx[2], qx[3]]});
    ...
    async.forEachSeries(Object.keys(arg.types), function(type, cb){
      ...
      self.transactionQueries.push({sql:sql, binds:[idr.baseHash(i2), arg.at, idr.hash(i2), idr.hash(i2), self.path, hash, arg.at, offset, (self.len - offset), ll[0], ll[1], qx[0], qx[1], qx[2], qx[3]]});
      cb();
    }, callback);
  });
}
