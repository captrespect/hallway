...
IJOD.prototype.addData = function(arg, callback) {
  ...
  this.startAddTransaction(function() {
    ...
    if (exports.debug) logger.debug("INSERT INTO ijod VALUES (%s, %s, %s, %s, %d, %d, %d) ON DUPLICATE KEY UPDATE path=VALUES(path), hash=VALUES(hash), at=VALUES(at), offset=VALUES(offset), len=VALUES(len)", idr.hash(arg.idr), idr.baseHash(arg.idr), self.path, hash, arg.at, offset, (self.len - offset));
    self.db.query("INSERT INTO ijod VALUES (?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE path=VALUES(path), hash=VALUES(hash), at=VALUES(at), offset=VALUES(offset), len=VALUES(len)", [idr.hash(arg.idr), idr.baseHash(arg.idr), self.path, hash, arg.at, offset, (self.len - offset)], callback);
  });
}
          ...
          function nextRange() {
            ...
            if (curBuf) {
              ...
              try {
                ...
                logger.error(E, curPath, rangeStr, curBuf);
                curRange = undefined;
                return;
              }
            }
...
exports.batchSmartAdd = function(entries, callback) {
  ...
  dal.acquire(function(err, db) {
    ...
    db.query("SELECT idr,hash FROM ijod WHERE ijod.idr IN (" +  entryInClause + ")", function(error, rows) {
