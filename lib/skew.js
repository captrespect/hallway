...
// TODO/BAG: Set the worker name? Or use node-mutex?
...
Skew.prototype.incrementErrors = function(key, callback) {
  // TODO/BAG
  //self.redisClient.hset('jobs:' + key, function(err) {
  //});
};

// TODO/BAG: Specify TTR as an argument?
// TODO/BAG: Maybe this could just take a task?
Skew.prototype.schedule = function(key, nextRun, payload, priority, callback) {
  ...
  console.log('XXX: Acquiring beanstalk for skew.schedule...', key);

  self.beanstalkPool.acquire(function(err, beanstalk) {
    if (err)
      return callback(new Error(err));

    console.log('XXX: Acquired beanstalk for skew.schedule.');
...
Skew.prototype.reserve = function(callback) {
  ...
  console.log('XXX: Acquiring beanstalk for skew.reserve...');
...
Skew.prototype.release = function(key, jobId, priority, delay, callback) {
  ...
  self.untrackRedisJob(key, function(err) {
    console.log('XXX: Acquiring beanstalk for skew.release...', key, jobId);

    self.beanstalkPool.acquire(function(err, beanstalk) {
      if (err)
        return callback(new Error(err));

      console.log('XXX: Acquired beanstalk for skew.release.');
...
Skew.prototype.destroy = function(key, jobId, callback) {
  ...
  self.untrackRedisJob(key, function(err) {
    console.log('XXX: Acquiring beanstalk for skew.destroy...', key, jobId);

    self.beanstalkPool.acquire(function(err, beanstalk) {
      if (err)
        return callback(new Error(err));

      console.log('XXX: Acquired beanstalk for skew.destroy.');
