...
// safely run the mapped functions
...
exports.get = function(name, data, base) {
  ...
  if(typeof map[name] === 'function') return condom(map[name],data,base,name);
...
exports.map = function(entry) {
  ...
  Object.keys(map).forEach(function(name){
    if(name == 'at') return;
    var x = (typeof map[name] === 'function') ? condom(map[name],entry.data,entry.idr,name) : entry.data[map[name]];
    if(x) ret[name] = x;
  });
  return ret;
}
