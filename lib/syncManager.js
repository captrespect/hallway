...
// Async init
SyncletManager.prototype.init = function(callback) {
  var self = this;
  dal.query("CREATE TABLE IF NOT EXISTS SyncSchedule (`key` VARCHAR(255) PRIMARY KEY, worker VARCHAR(255), nextRun BIGINT UNSIGNED, state INT, task TEXT)", [], function(error) {
...
SyncletManager.prototype.loadSynclets = function() {
  ...
  function synclets(connector) {
    ...
    var sjs = JSON.parse(fs.readFileSync(path.join(__dirname, 'services', connector, 'synclets.json')));
    for (var i = 0; i < sjs.synclets.length; i++) {
      var sname = sjs.synclets[i].name;
      var spath = path.join("services", connector, sname);
...
SyncletManager.prototype.schedule = function(task, timeToRun) {
  ...
  dal.query(sql, binds, function(error, rows) {
    ...
    dal.query("INSERT INTO SyncSchedule (`key`, worker, nextRun, state, task) VALUES(?, ?, ?, 0, ?) ON DUPLICATE KEY UPDATE nextRun=VALUES(nextRun),task=VALUES(task),state=0", [key, lconfig.workerName, timeToRun, JSON.stringify(task)], function(error) {
      if (error) {
...
SyncletManager.prototype.scanAndRun = function() {
  ...
  dal.query(sql, binds, function(error, rows) {
    if (error) {
      logger.error("There was an error trying to scanAndRun");
      return setNextScan();
    }
    async.forEach(rows, function(row, cb) {
      ...
      if (ranRows > 0) return;
      dal.query("SELECT MIN(nextRun) AS nextRun FROM SyncSchedule WHERE worker=? AND state=0", [lconfig.workerName], function(error, rows) {
        if (error) {
        ...
        logger.debug("Setting next run timeout to %d - %j", scanTimeout, rows[0]);
