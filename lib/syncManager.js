...
var one_day_in_milliseconds = 24 * 60 * 60 * 1000; // Broken down so you can visualize it, it's 86,400,000ms
...
SyncletManager.prototype.schedule = function(task, timeToRun, cbDone) {
  ...
  // Make sure we have tolerance information on the task
  if (!task.tolerance) {
    task.tolerance = {
  ...
  dal.query(sql, [self.getKey(task)], function(error, rows) {
    ...
    if (timeToRun === undefined || timeToRun === null || timeToRun <= 0) {
      ...
      logger.debug("nextRun(%d) tolerance(%j)", nextRun, task.tolerance);
      // If it's not paging we're going to see if we need to apply tolerance
      if (nextRun > PAGING_TIMING && task.tolerance) {
        var factor = (BACKOFF_FACTOR ^ task.tolerance.current) + 1;
        var backoff = (1/(factor + 1)) * (factor * (factor + 1) / 2);
        nextRun = backoff * nextRun;
        // We max at one day
        if (nextRun > one_day_in_milliseconds) nextRun = one_day_in_milliseconds;
        logger.info("Applied a tolerance backoff to %s with a level of %d", key, task.tolerance.current);
      }
...
SyncletManager.prototype.runTask = function(task, callback) {
  ...
  function cbDone(err, response)
  {
    ...
    if (task.tolerance.averages.length > 5) {
      task.tolerance.averages.forEach(function(avg) { average += avg; });
    ...
    if (self.total > average * 1.1) {
      // We are over the averate let's bump the tolerance down so we run more
      task.tolerance.current --;
      if (task.tolerance.current < 0) task.tolerance.current = 0;
    } else if (self.total <= average * 0.90) {
    ...
    self.updateState(self.getKey(task), 3, function(error) {
      if (!err) {
        ...
        dal.query("UPDATE SyncSchedule SET worker=NULL,errorCount=errorCount+1, lastError=? WHERE `key`=?", [estr.substr(0, 255), self.getKey(task)], function(error) {
          if (self.completed && task.count > 0) return self.completed(response, task, callback); // if there was data given, process it!
          self.schedule(task);
          callback(err);
        });
      }
