  ...
  function cbDone(err, response)
  {
    ...
    self.updateState(self.getKey(task), 3, function(error) {
      if (!err) {
        ...
        instruments.increment("synclet.error").send();
        dal.query("UPDATE SyncSchedule SET worker=NULL,errorCount=errorCount+1, lastError=? WHERE `key`=?", [err.toString().substr(0, 255), self.getKey(task)], function(error) {
          self.schedule(task);
        });
      }
      callback(err);
