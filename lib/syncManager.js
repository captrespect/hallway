  ...
  async.series([
    function(cb) {
      ...
      self.skew.clearWorkerJobs(function(err, clearedJobs) {
        // TODO/BAG: Exit here if error?
        if (!err && clearedJobs > 0) {
    ...
    // TODO/BAG: Do something if we error here
...
SyncletManager.prototype.schedule = function(task, timeToRun, cbDone) {
  ...
  self.skew.isJobRunning(self.getKey(task), function(err, running) {
    ...
    // TODO/BAG: Is it OK to reschedule a job if it's in state 3?
    //           If yes, find out what state the job is in here.
    if (running) {
