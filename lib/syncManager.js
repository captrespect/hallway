...
SyncletManager.prototype.init = function(sman, callback) {
  ...
  dal.acquire(function(error, db) {
    db.query("CREATE TABLE IF NOT EXISTS SyncSchedule (`key` VARCHAR(255) PRIMARY KEY, worker VARCHAR(255), nextRun BIGINT UNSIGNED, state INT, task TEXT)", function(error) {
...
SyncletManager.prototype.schedule = function(task, timeToRun) {
  ...
  dal.acquire(function(error, db) {
    db.query(sql, binds, function(error, rows) {
      ...
      db.query("INSERT INTO SyncSchedule (`key`, worker, nextRun, state, task) VALUES(?, ?, ?, 0, ?) ON DUPLICATE KEY UPDATE nextRun=VALUES(nextRun),task=VALUES(task),state=0", [key, lconfig.workerName, timeToRun, JSON.stringify(task)], function(error) {
        if (error) {
  ...
  dal.acquire(function(error, db) {
    db.query(sql, binds, function(error, rows) {
      ...
      async.forEach(rows, function(row, cb) {
        ...
        if (ranRows > 0) return;
        db.query("SELECT MIN(nextRun) AS nextRun FROM SyncSchedule WHERE worker=? AND state=0", [lconfig.workerName], function(error, rows) {
          if (error) {
...
SyncletManager.prototype.syncNow = function(key, cb) {
  ...
  dal.acquire(function(error, db) {
    db.query(sql, binds, function(err) {
