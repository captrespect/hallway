...
SyncletManager.prototype.init = function(sman, callback) {
  serviceManager = sman;
  var self = this;
  self.db.query("CREATE TABLE IF NOT EXISTS SyncSchedule (`key` VARCHAR(255) PRIMARY KEY, nextRun INT, state INT, task TEXT)", function(error) {
    self.loadSynclets();
    var res = self.db.query("SELECT task FROM SyncSchedule WHERE state != 0");
...
SyncletManager.prototype.schedule = function(task, timeToRun) {
  ...
  this.db.query("SELECT * FROM SyncSchedule WHERE `key`=?", [key], function(error, rows) {
    if (error) {
    ...
    self.db.query("REPLACE INTO SyncSchedule VALUES(?, ?, 0, ?)", [key, timeToRun, JSON.stringify(task)], function(error) {
      if (error) {
...
SyncletManager.prototype.scanAndRun = function() {
  ...
  var ranRows = 0;
  var res = this.db.query("SELECT * FROM SyncSchedule WHERE nextRun <= ? AND state=0", [Date.now()]);
  res.on("error", function(error) {
  ...
  res.on("row", function(row) {
    ++ranRows;
    self.db.query("UPDATE SyncSchedule SET state=1 WHERE `key`=?", [row.key], function(error) {
      if (error) {
...
SyncletManager.prototype.cleanup = function(task, cb) {
  ...
  this.db.query("DELETE FROM SyncSchedule WHERE key=?", [this.getKey(task)], cb);
};
...
SyncletManager.prototype.runTask = function(task, callback) {
  ...
  */
  ...
  this.db.query("UPDATE SyncSchedule SET state=2 WHERE `key`=?", [this.getKey(task)], function(error) {
    if (error) {
...
exports.syncNow = function (service, auth, callback, syncletId) {
  ...
  if (!syncletManager.synclets[service]) return callback("no synclets like that installed");
  async.forEachSeries(syncletManager.synclets[service], function (synclet, cb) {
...
exports.scheduleAll = function(callback) {
  ...
  async.forEach(Object.keys(map), function(service, cb){ // do all services in parallel
    // We only want services with synclets
    if(!map[service].synclets || !map[service].base) return cb();
