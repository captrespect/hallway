...
exports.sync = function(pi, cb) {
  request.get({uri:'https://api.foursquare.com/v2/users/self?v=20120413&oauth_token='+pi.auth.accessToken, json:true}, function(err, resp, js) {
    ...
    if(resp.statusCode != 200) return cb(new Error("status code "+resp.statusCode+" "+util.inspect(js)));
    if(!js || !js.response || !js.response.user) return cb(new Error("missing response.user: "+util.inspect(js)));
