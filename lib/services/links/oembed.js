...
exports.pump = function(cset, callback) {
  ...
  function saver(task, data) {
    ...
    task.entries.forEach(function(entry){
      ...
      entry.q = [entry.q,data.title,(data.type=="link")?data.url:''].join(" "); // neato
    });
  }
  ...
  var q = async.queue(function(task, cb){
    ...
    ijod.getOne(task.idr, function(err, entry) {
      ...
      // now do oembed, have max fail-safe timer since the oembeds don't always internally timeout sanely
      task.timer = setTimeout(function(){
        logger.debug("timing out",task.url);
        task.timer = false;
// TODO: we used to cache errors, and now we always retry, there might be a happier medium
//        saver(task, {type:'link', url:task.url, err:'oEmbed timed out.'}); // save a blank one since it broke
        cb();
      ...
      oembed(task.url, function(data){
        ...
        if(!data) return async.nextTick(cb); // bail if no oembed
...
//XXX PhotoBucket's oembed endpoint requires cookies and redirects to infinity
  ...
  async.forEachSeries(stack, function (u, cb) {
    ...
    request.get({
      ...
      if(err || !body || !body.type) {
        logger.debug("oembed failed",
          urllib.format(up), err || resp.statusCode);
      }
      if(err || !body || !body.type) return cb(); // continue on to next one
      if(body.type == "image") body.type = "photo"; // yfrog does this, image is not in the spec
