...
exports.getPageLikes = function(arg, cbEach, cbDone, cbTime) {
    var fb = this;
    var uri = 'https://graph.facebook.com/fql?q=SELECT%20page_id,created_time%20FROM%20page_fan%20WHERE%20uid=me()AND%20created_time%20%3E%20' + arg.since + '&access_token=' + arg.accessToken;
    getOne(uri,
    function(err, pages) {
        if (err || !Array.isArray(pages.data)) return cbDone(err);
        var ids = [];
        pages.data.forEach(function(page) {
            ids.push(page.page_id);
            cbTime(page.created_time);
        });
        if(ids.length == 0) return cbDone();
        getOne('https://graph.facebook.com/?access_token=' + arg.accessToken + '&ids=' + ids.join(','),
        function(err, js) {
            if (err || typeof js != 'object') return cbDone(err);
...
exports.getUrlLikes = function(arg, cbDone) {
	...
	var uri = 'https://graph.facebook.com/fql?q=SELECT%20url%20FROM%20url_like%20WHERE%20user_id%20=%20'+arg.id+'%20LIMIT%20'+URL_LIMIT+'%20OFFSET%20'+arg.offset+'&access_token=' + arg.accessToken;
	var done = false;
	request.get({uri:uri, json:true, timeout:timeout}, function(err, resp, json) {
		...
		processUrls(function(doneYet, arg){
			if (doneYet) cbDone(null,arg.urls);
			else {
				arg.offset += URL_LIMIT;
				fb.getUrlLikes(arg, cbDone);
			}
		});
	});
}
...
exports.getStreamLikes = function(arg, cbEach, cbDone, cbTime) {
	var uri = 'https://graph.facebook.com/fql?q=SELECT%20object_id%20FROM%20like%20WHERE%20user_id%20=%20'+arg.id+'&access_token='+arg.accessToken;
	getOne(uri, function(err, resp){
		if(err || !Array.isArray(resp.data) || resp.data.length == 0) return cbDone(err);
		var ids = [];
		cbTime(resp.data[0].object_id);
		resp.data.every(function(stream_like){
			ids.push(stream_like.object_id);
			ids.push(arg.id+'_'+stream_like.object_id);
			if (stream_like.object_id == arg.newestObjID) return false;
			else return true;
		});
		if(ids.length == 0) return cbDone();
		uri = 'https://graph.facebook.com/?ids='+ids.join(',')+'&access_token='+arg.accessToken;
		getOne(uri, function(err, resp){
			if(err || typeof resp != "object") return cbDone(err);
