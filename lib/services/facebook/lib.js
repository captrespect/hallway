...
function apiUrl(arg, path, params) {
  return urllib.format({
    protocol: 'https',
    host: API_HOST,
    pathname: path,
    query: lutil.extend(params, {
...
function getData(arg, path, cbEach, cbDone) {
  ...
  arg.uri = apiUrl(arg, path, params);
...
exports.getFriends = function(arg, cbEach, cbDone) {
  var uri = apiUrl(arg, '/' + arg.id + '/friends', {limit: PAGE_SIZE});
  getPage(uri, function(err, friends) {
    ...
    var friendsURI = apiUrl(arg, '/', {ids: ids.join(',')});
...
exports.getPageLikes = function(arg, cbDone) {
  ...
  exports.getFQL(arg, function(err, pages){
		...
		processPageLikes(data, arg, function(err, done, arg){
      if (err) return cbDone(err);
			if (done) {
...
function processPageLikes(data, arg, cbDone){
	var done = false;
  if (data.length === 0) {
    cbDone(true, arg);
  } else {
    ...
    var uri = apiUrl(arg, '/', {ids: pageIDs.join(',')});
...
exports.getUrlLikes = function(arg, cbDone) {
  ...
  exports.getFQL(arg, function(err, urls) {
		...
		processUrls(urls.data, arg, function(err, done, arg){
      if (err) return cbDone(err);
...
exports.getStreamLikes = function(arg, cbDone) {
  ...
  exports.getFQL(arg, function(err, likes) {
		...
		processStreamLikes(data, arg, function(err, done, arg){
      if (err) return cbDone(err);
...
function processStreamLikes(likes, arg, cbDone){
  ...
  var uri = apiUrl(arg, '/', {ids: ids.join(',')});
  getPage(uri, function(err, resp){
    if (err || !resp) {
      err = err || new Error("Invalid response");
      return cbDone(err);
...
exports.getPerson = function(arg, cbEach, cbDone) {
  ...
  var uri = apiUrl(arg, '/' + arg.id, {fields: ALL_USER_FIELDS});
...
exports.getAlbums = function (arg, cbDone) {
  ...
  arg.query = 'SELECT object_id, modified FROM album' +
...
exports.getPostPage = function(arg, cbDone) {
  ...
  if (arg.since) params.since = arg.since;
  var uri = apiUrl(arg, '/' + arg.id + '/' + arg.type, params);
  getPage(uri, cbDone);
};
...
exports.getObjects = function(arg, cbDone) {
  ...
  var uri = apiUrl(arg, '/', {ids: arg.ids.join(',')});
...
exports.getProfile = function(arg, cbDone) {
  var uri = apiUrl(arg, '/me', {fields: ALL_USER_FIELDS});
  getPage(uri, cbDone);
};
...
exports.getFQL = function(arg, cbDone) {
  var uri = apiUrl(arg, '/fql', {q: arg.fql});
