...
// Get page likes
exports.getPageLikes = function(arg, cbEach, cbDone) {
    var fb = this;
	var uri = 'https://graph.facebook.com/fql?q=SELECT%20page_id,created_time%20FROM%20page_fan%20WHERE%20uid='+arg.id+'&access_token=' + arg.accessToken;
    getOne(uri,function(err,pages){
        if(err || !Array.isArray(pages.data)) return cbDone(err);
        var ids = [];
        pages.data.forEach(function(page){ ids.push(page.id) });
        getOne('https://graph.facebook.com/?access_token='+arg.accessToken+'&ids='+ids.join(','), function(err, js){
          if(err || typeof js != 'object') return cbDone(err);
          Object.keys(js).forEach(function(key){ cbEach(js[key]) });
          cbDone();
...
/*// fetching all page likes
exports.getLikes = function(arg, cbEach, cbDone){
	var since = (arg.since)?"&since="+arg.since:"";
  arg.uri = (arg.page)?arg.page:'https://graph.facebook.com/'+arg.id+'/likes?limit=500&access_token=' + arg.accessToken + '&date_format=U'+since;
  getDatas(arg, cbEach, cbDone);
}
*/
