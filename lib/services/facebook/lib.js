...
exports.getUrlLikes = function(arg, cbEach, cbDone) {
	var fb = this;
	var uri = 'https://graph.facebook.com/fql?q=SELECT%20url%20FROM%20url_like%20WHERE%20user_id%20=%20'+arg.id+'&access_token=' + arg.accessToken;
	getOne(uri, function(err, resp){
		if(err || !Array.isArray(resp.data)) return cbDone(err);
		var count = 0;
		resp.data.every(function(url_like){
			cbEach(url_like, count);
			count++;
			if (url_like.url == arg.newestUrl) return false;
			else return true;
		});	
		cbDone();
	});
