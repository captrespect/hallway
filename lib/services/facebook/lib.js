...
// Get stream likes (this includes likes of things not in the homestream. Maybe the general stream?)
exports.getStreamLikes = function(arg, cbEach, cbDone) {
	...
	getOne(uri, function(err, resp){
		if(err || !Array.isArray(resp.data)) return cbDone(err);
		var ids = [];
		resp.data.forEach(function(stream_like){
			ids.push(stream_like.object_id);
			ids.push(arg.id+'_'+stream_like.object_id);
		});
		uri = 'https://graph.facebook.com/?metadata=1&ids='+ids.join(',')+'&access_token='+arg.accessToken;
		getOne(uri, function(err, resp){
			if(err || !Array.isArray(resp.data)) return cbDone(err);
...
function writeBase (obj) {
	var type = object.metadata.type || 'post';
	v 
}
