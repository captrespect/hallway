...
exports.sync = function(pi, cbDone) {
  ...
  async.waterfall([function(cb){
  	fb.getUrlLikes({id:"me()", accessToken:pi.auth.accessToken, newestUrl:newestUrl},function(err, urls){
  		if (urls && urls[0]) resp.config['newestUrl'] = urls[0];
  		if(err) errors.push(err);
console.log('printing urls');
		console.log(urls);
console.log('ending urls');
  		cb();
  	});	
  },function(cb){
