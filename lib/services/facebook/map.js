...
function participants(data) {
  ...
  if(data.to && Array.isArray(data.to.data)) data.to.data.forEach(function(to){ ret[to.id] = ret[to.id] || {}});
  if(data.likes && Array.isArray(data.likes.data)) data.likes.data.forEach(function(like){ ret[like.id] = ret[like.id] || {}});
  if(data.comments && Array.isArray(data.comments.data)) data.comments.data.forEach(function(comment){ if(comment.from) ret[comment.from.id] = ret[comment.from.id] || {}});
  return (Object.keys(ret).length > 0) ? ret : undefined;  
}

exports.post = {
  at: function(data) { return (data.updated_time||data.created_time) * 1000 },
  ...
  author: function(data) {
    ...
    ret.photo = 'https://graph.facebook.com/' + data.from.id + '/picture?type=large';
    return ret;
  },
...
exports.photo = {
  ...
  oembed: function(data) {
    ...
    ret.url = data.source||data.picture;
    ret.thumbnail_url = data.picture;
    ret.provider_name = 'facebook';
...
exports.url = {
  url: function(data) { return [data.url]},
  id: function(data) {
