...
//TODO setup dialogue to retrieve user and password
exports.getAccessToken = function (req, res, callback) {
	var email = 'charlie@singly.com';
	var password = 'password';	
	request.post({uri:apiBase+'/v2/authentication/login', body:'email='+email+'&password='+password}, // json: true},
	function(err, resp, body) {
		try {body = JSON.parse(body);}
		catch(err) {}
			return callback(null, body.token, body.athlete.id);
	});
}

exports.getProfile = function(arg, callback){
	exports.apiCall('/v2/athletes/'+arg.athlete_id, {token:arg.token}, function(err, resp, json){
		if(err || resp.statusCode !== 200 || !json) return callback(err, resp, json);
		callback(undefined, resp, json);
	});
}
...
//Note Strava calls these maps, but they are more accurately described as paths (time-sequenced series of latlngs)
//Called paths here to be consistent with runkeeper.
exports.getPaths = function(token, ridesWithDetails, callback) {
	var paths = [];
	var params = {token:token};
	ridesWithDetails.forEach(function(ride){
		params.id = ride.id;
		exports.apiCall('/v1/rides/'+ride.id+'/map_details', params, function(err, resp, json){
			if(err || resp.statusCode !== 200 || !json) return callback(err, resp, json);
			paths.push(json);
		});
	});
