...
//  console.error("TWITTER COOKIE get "+JSON.stringify(ret));
  return ret;
}
function setCookie(serial, res, js)
{
//  console.error("TWITTER COOKIE set "+JSON.stringify(js));
 ...
 module.exports = function (key, secret, callbackURI) {
   ...
   CLIENT.apiCall = function (method, path, params, callback) {
     ...
     // req is apparently undefined here, meaning we have no way to abort it if it times out, @#%$@#^!@#%$@!!!!
     setTimeout(function(){
       if(!active) return;
       active = false;
       callback("timed out");
       ...
       CLIENT.oauth.getOAuthRequestToken(
         ...
         function (error, oauth_token, oauth_token_secret, oauth_authorize_url, additionalParameters) {
           if (!error) {
             ...
             res.redirect('http://api.twitter.com/oauth/'+auth+'?oauth_token=' + oauth_token);
           } else {
             callback(error, null);
           }
         }
