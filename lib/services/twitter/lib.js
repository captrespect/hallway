...
exports.getFFchunk = function(pi, arg, cbDone)
{
  ...
  getOne(pi.tc, arg, function(err,js){
      ...
      var users = [];
      var ids = js.ids.slice(((arg.slice > 10) ? arg.slice-10 : arg.slice), 1700); // three passes to do 5k, drift back a bit to overlap more efficiently
      exports.getUsers(pi, ids, function(user){users.push(user)}, function(err){
...
exports.getRelated = function(pi, arg, cbEach, cbDone) {
    ...
    getOne(pi.tc, {token:pi.auth.token, path:"/related_results/show/"+arg.id+".json"},function(err,related){
        if(err || !Array.isArray(related)) return cbDone(err);
        getOne(pi.tc, {token:pi.auth.token, path:"/statuses/"+arg.id+"/retweeted_by.json"},function(err,retweeted){
...
function getOnePublic(arg, cb) {
    ...
    delete arg.path;
    api.query = arg;
    request.get({uri:url.format(api), timeout:timeout}, function(err, resp, body) {
        var js;
        try{
            if(err) throw err;
            js = JSON.parse(body);
